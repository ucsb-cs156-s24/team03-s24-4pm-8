...
describe("UCSBDiningCommonsMenuItemIndexPage tests", () => { 
    ...
    test("renders three UCSBDiningCommonsMenuItems correctly for regular user", async () => {
        ...
        const name = screen.getByText("Cream of Broccoli Soup (v)"); //Entree Specials
        expect(name).toBeInTheDocument();

        const station = screen.getByText("Greens & Grains"); //Entree Specials
    ...
    test("renders empty table when backend unavailable, user only", async () => {
        ...
        axiosMock.onGet("/api/diningcommonsmenuitem/all").timeout();
        ...
        expect(errorMessage).toMatch("Error communicating with backend via GET on /api/diningcommonsmenuitem/all");
        restoreConsole();
    ...
    test("what happens when you click delete, admin", async () => {
        ...
        axiosMock.onGet("/api/diningcommonsmenuitem/all").reply(200, ucsbDiningCommonsMenuItemsFixtures.threeDiningCommonsMenuItems);
        axiosMock.onDelete("/api/diningcommonsmenuitem").reply(200, "UCSBDiningCommonsMenuItem with id 1 was deleted");
        ...
        expect(axiosMock.history.delete[0].url).toBe("/api/diningcommonsmenuitem");
        expect(axiosMock.history.delete[0].url).toBe("/api/diningcommonsmenuitem");
