...
describe("UCSBDiningCommonsMenuItemEditPage tests", () => {
    ...
    describe("when the backend doesn't return data", () => {
        ...
        beforeEach(() => {
            ...
            axiosMock.onGet("/api/diningcommonsmenuitem", { params: { id: 1 } }).timeout(); // UCSBDiningCommonsMenuItem
        });
    ...
    describe("tests where backend is working normally", () => {
        ...
        beforeEach(() => {
            axiosMock.reset();
            axiosMock.resetHistory();
            axiosMock.onGet("/api/currentUser").reply(200, apiCurrentUserFixtures.userOnly); // UCSBDiningCommonsMenuItemEditPage
            axiosMock.onGet("/api/systemInfo").reply(200, systemInfoFixtures.showingNeither);
            axiosMock.onGet("/api/diningcommonsmenuitem", { params: { id: 1 } }).reply(200, {
                id: 1,
                diningCommonsCode: "ortega",
                name: "Baked Pesto Pasta with Chicken",
                station: "Entree Specials"
            });
            axiosMock.onPut('/api/diningcommonsmenuitem').reply(200, {
                id: "1",
                diningCommonsCode: "portola",
                name: "Baked Pesto Pasta with Bacon",
                station: "Entree Specialties"
            });
        });
        ...
        test("Is populated with the data provided", async () => {
            ...
            fireEvent.change(nameField, { target: { value: 'Baked Pesto Pasta with Bacon' } }); // stationField
            fireEvent.change(stationField, { target: { value: 'Entree Specialties' } });
            fireEvent.click(submitButton);

            await waitFor(() => expect(mockToast).toBeCalled());
            expect(mockToast).toBeCalledWith("UCSBDiningCommonsMenuItem Updated - id: 1 name: Baked Pesto Pasta with Bacon");
